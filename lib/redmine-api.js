...
Api.prototype.getIssue = function(callback){
  ...
  that.getIdIssue(function(id){
    var pattern_id = new RegExp(/([0-9]+$)/gi);
    var pattern_format = new RegExp(/[^(?!\/)]([a-z]+$)/gi);
    
    that.issues.id = id.match(pattern_id);
    that.format = that.contentType.match(pattern_format)[0];
